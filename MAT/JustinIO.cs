...
namespace JustinIO {
    ...
    class CommPortEvent
    {
        public delegate void JustinIOReceivedEventHandler(object sender, string dataStr);
        public delegate void JustinIOErroEventHandler(object sender, Exception ex);
...
　　										当接收缓冲区仍有剩余字节时RTS为ON ,否则缺省为OFF*/
		...
		public void Open()
		{
            ...
            string PortNumTemp = PortNum;
            int comNum = Int32.Parse(PortNum.Substring(3));
            if (comNum>9)
            {
                PortNumTemp = string.Format("\\\\.\\COM{0}", comNum);
            }
			 // 打开串口 OPEN THE COMM PORT.
            hComm = CreateFile(PortNumTemp  ,GENERIC_READ | GENERIC_WRITE,0, 0,OPEN_EXISTING,0,0);
        ...
        /************************************************************************/
        /* add the interface by Lirilu                                          */
        /************************************************************************/
        public string ReadExisting()
        {
             string response = string.Empty;
             response = Encoding.ASCII.GetString(this.Read(512)); //读取512个字节缓存
             return response;
        }
        ...
        {
            Write(Encoding.ASCII.GetBytes(string.Format("{0}\0",text)));
        }

        public event CommPortEvent.JustinIOReceivedEventHandler DataReceived;
        public event CommPortEvent.JustinIOErroEventHandler DataReceiveErro;
        Thread thread;

        /************************************************************************/
        /* add the interface by Lirilu                                          */
        /************************************************************************/
        public void StartAutoReciveData()
        {
            thread = new Thread(ThreadToReadData);
        ...
        /* add the interface by Lirilu                                          */
        /************************************************************************/
        public void StopAutoReciveData()
        {
            isRuning = false;
        ...
        private void ThreadToReadData()
        {
            while(isRuning)
            {
                ...
                try
                {
                    response = Encoding.ASCII.GetString(this.Read(1024)); //读取1024个字节缓存
                    Thread.Sleep(50);
                }
                catch (Exception ex)
                {
                    if (DataReceiveErro != null)
                    {
                        DataReceiveErro(this, ex);
                    }
                }
                if (DataReceived != null && isRuning != false)
                {
                ...
                {
                    System.Console.WriteLine(string.Format("{0}->{1}",PortNum,response));
                }
            }
        }
